...
import com.cn.flume.tail.util.BigCursor;
import com.cn.flume.tail.util.Clock;

public class BigTailDateSource  extends AbstractSource implements Configurable, PollableSource {

	private static final Logger LOG = LoggerFactory.getLogger(BigTailDateSource.class);
	...
	public void configure(Context context) {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				ydtMap = null;
				BigCursor ccursor = getCursorByFileName(asFileName);
				if(ccursor != null) {
				...
				if(ccursor == null) {
					...
					BigCursor newCursor = new BigCursor(sync, file, 0, file.length(), file.lastModified());
					addCursor(newCursor);
	...
	synchronized public BigCursor getCursorByFileName(String fileName) {
		BigCursor cursor = null;
		for(BigCursor c : cursors) {
